            ...
            {open && (
                <div className="px-4 pb-3 border-t border-gray-100">
                    <div className="text-xs text-gray-400 mt-2 mb-1">
                        Начало: {formatTime(step.started_at)}
                    </div>
                    {step.error && (
                    ...
                    <JsonBlock data={step.input} label="Входные данные" />
                    <JsonBlock data={step.output} label="Результат" />
                    {step.substeps && step.substeps.length > 0 && (
                        <div className="mt-2">
                            <div className="text-xs font-medium text-gray-600 mb-1">
                                Подробности ({step.substeps.length})
                            </div>
                            {step.substeps.map((sub, i) => (
                                <JsonBlock key={i} data={sub} label={`Слайд ${(sub as any)?.slide_index ?? i}`} />
                            ))}
                        </div>
                    )}
                </div>
...
function GenerationDetail({ logId }: { logId: string }) {
    ...
    useEffect(() => {
        setLoading(true)
        fetch(`/api/v1/ppt/presentation/logs/${logId}`)
            .then(r => r.json())
    ...
    if (!detail) return <div className="text-sm text-red-500 py-4">Не удалось загрузить</div>

    return (
        <div className="mt-3 ml-4 mb-6">
            <div className="flex items-center gap-6 text-xs text-gray-500 mb-3 pb-2 border-b">
                <span>ID: <span className="font-mono">{detail.presentation_id || '—'}</span></span>
...
export default function GenerationLogsPage() {
    ...
    return (
        <div className="min-h-screen bg-white p-6 max-w-5xl mx-auto" style={{ fontFamily: 'system-ui, sans-serif' }}>
            <h1 className="text-2xl font-bold mb-1">Логи генерации</h1>
            <p className="text-sm text-gray-500 mb-6">Пошаговая трассировка каждой генерации презентации</p>

            {loading ? (
                <div className="text-center text-gray-400 py-12">Загрузка...</div>
            ) : logs.length === 0 ? (
                <div className="text-center text-gray-400 py-12">Генераций пока нет</div>
            ) : (
                <div className="space-y-2">
                    {logs.map(lg => (
                        <div key={lg.id}>
                            <button
                                onClick={() => setExpandedId(expandedId === lg.id ? null : lg.id)}
                                className={`w-full text-left px-4 py-3 border rounded-lg hover:bg-gray-50 transition ${expandedId === lg.id ? 'border-blue-400 bg-blue-50' : 'border-gray-200'}`}
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-gray-500">
                                        <span>{lg.steps_count} шагов</span>
